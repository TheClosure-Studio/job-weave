...
export default function Streak({ items = [], showBreakdown = true, onCheckIn, isCheckedIn, title, description, onDayClick, todayCount, onAdjustCount }) {
    ...
    useEffect(() => {
        ...
        const today = new Date(todayStr); // Ensure this parses correctly as local midnight or close enough for relative math
        // However, getISTDate returns YYYY-MM-DD. `new Date("YYYY-MM-DD")` is UTC usually.
        // Better to just offset by days from 'todayStr' string logic or use date object carefully.

        // Simplest: Create a helper loop based on timestamps? No, timezone issues.
        // Let's rely on date objects constructed from the IST string, 
        // but we need to match the 'YYYY-MM-DD' output of getISTDate.

        for (let i = 0; i < 364; i++) {
            const d = new Date(todayStr);
            d.setDate(d.getDate() - (363 - i));

            // Convert that back to IST string to match our activityMap keys
            // Since we constructed it from an IST string (interpreted as UTC by JS default for YYYY-MM-DD usually),
            // we should just stick to the string manipulation or standard date.
            // Actually `new Date("2023-01-01")` is UTC. `new Date("2023/01/01")` is local.
            // We want to generate the string keys that match `getISTDate` format.

            // Let's use `toISOString` split if we accept the date object is shifting in UTC.
            // IF todayStr is "2026-01-31", new Date is 2026-01-31T00:00:00Z.
            // Subtracting days keeps it Z. toISOString().split returns YYYY-MM-DD.
            // This works for simple day subtraction effectively.
            const dateString = d.toISOString().split('T')[0];
            const dateObj = d; // Just for reference
            ...
            data.push({ date: dateObj, level, count });
        }
